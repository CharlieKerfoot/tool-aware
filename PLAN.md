...
1. Scaffold the repo with the 4 source files + workflow YAML
2. fetch.py — hit both APIs, return merged deduped list of {name, url, description, stars, hn_points, age_days}
3. score.py — send list to Claude with rubric + your profile, parse JSON response
4. snooze.py — on each run, check snooze.json for items due this week, append to digest
5. notify.py — format and send via smtplib
weekly.yml — cron schedule, secrets wiring, auto-commit snooze.json if changed
6. A separate snooze_tool.yml — workflow_dispatch with tool, url, weeks inputs, runs snooze.py --add
